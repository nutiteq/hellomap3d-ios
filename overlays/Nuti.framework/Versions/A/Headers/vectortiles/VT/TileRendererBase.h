...
namespace Nuti {
	namespace VT {
		...
		public:
			TileRendererBase(std::mutex& mutex, const std::shared_ptr<GLExtensions>& glExtensions, float scale);
			virtual ~TileRendererBase() = default;

			void setViewState(const cglib::mat4x4<double>& projectionMatrix, const cglib::mat4x4<double>& cameraMatrix, float zoom);
		...
		private:
			...
			struct CompiledGeometry {
				...
				CompiledGeometry() : vertexGeometryVBO(0), vertexAttribsVBO(0), indicesVBO(0), geometryVAO(0) { }
			};
			...
			void renderTileGeometry(const TileId& tileId, const TileId& sourceTileId, float blend, float opacity, const std::shared_ptr<TileGeometry>& geometry);
			...
			virtual GLuint createTexture(int width, int height);
			virtual GLuint createTexture(int width, int height, const std::uint32_t* data);

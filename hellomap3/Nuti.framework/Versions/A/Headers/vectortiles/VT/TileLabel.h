...
namespace Nuti { namespace VT {
	...
	public:
		...
		void mergeGeometries(const std::shared_ptr<TileLabel>& label);
	...
	private:
		...
		std::shared_ptr<Placement> findClippedPlacement(const ViewState& viewState, const VerticesList& verticesList) const;

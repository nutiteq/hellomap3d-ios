...
#ifndef _NUTI_TEXTURECACHE_H_
#define _NUTI_TEXTURECACHE_H_
...
namespace Nuti {
    ...
    public:
		...
		std::shared_ptr<Texture> get(const T& id) const;
    ...
    template <typename T>
	std::shared_ptr<Texture> TextureCache<T>::get(const T& id) const {
        std::lock_guard<std::mutex> lock(_mutex);
    ...
    template <typename T>
	void TextureCache<T>::invalidate(const T& id, std::chrono::system_clock::time_point expirationTime) {
        std::lock_guard<std::mutex> lock(_mutex);
